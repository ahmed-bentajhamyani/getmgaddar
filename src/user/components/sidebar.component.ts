...
@Component({
    ...
    <aside class="flex flex-col flex-shrink-0 p-3 shadow sticky top-0" style="width: 280px; height: 100vh;">
        <a routerLink="/" class="text-center text-4xl font-bold my-3">
            Get Mgaddar
        </a>
        <hr>
        
        <ul class="flex flex-col justify-start items-center w-full flex-grow">
            <li class="w-full mt-2 py-3 rounded-md cursor-pointer select-none transition-all ease-in-out duration-500" (click)="activateClass(1)"
            ...
            <li class="w-full mt-2 py-3 rounded-md cursor-pointer select-none transition-all ease-in-out duration-500" (click)="activateClass(2)" [ngClass]="{'text-white bg-rose-500 hover:bg-rose-700' : selectedOne === 2, 'text-gray-800 hover:bg-rose-500 hover:text-white' : selectedOne !== 2}">
